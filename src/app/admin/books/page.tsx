    ...
    <div style={{ backgroundColor: '#FAF7F2' }} className="min-h-screen flex">
      ...
      {editingBook && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto p-6">
            <h3 style={{ fontFamily: 'var(--font-playfair), serif', color: '#2D5016' }} className="text-xl font-bold mb-4">Edit Book</h3>
            <form onSubmit={handleSaveEdit} className="space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="button" onClick={() => setEditingBook(null)}
                  ...
                  style={{ borderColor: '#E8E0D5', color: '#666' }}>
                  Cancel
                </button>
              </div>
            </form>
          ...
          <nav className="space-y-1">
            {adminLinks.map(({ href, label, icon }) => (
              <Link key={label} href={href}
                ...
                style={{ color: '#FAF7F2', backgroundColor: label === 'Books' ? 'rgba(255,255,255,0.15)' : 'transparent' }}>
      ...
      <div className="flex-1 p-8 overflow-auto">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 style={{ fontFamily: 'var(--font-playfair), serif', color: '#2D5016' }} className="text-3xl font-bold mb-1">Books</h1>
          ...
          <Link href="/admin" className="text-sm px-4 py-2 rounded-xl font-medium text-white transition-opacity hover:opacity-90"
            style={{ backgroundColor: '#2D5016' }}>+ Add Book</Link>
        </div>
        ...
        <div className="mb-4 relative">
          ...
          <input
            ...
            style={{ border: '1.5px solid #E8E0D5', backgroundColor: 'white' }}
          />
        </div>

        {/* Table */}
        <div className="rounded-2xl shadow-sm overflow-hidden" style={{ backgroundColor: 'white', border: '1px solid #E8E0D5' }}>
          {loading ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {books.map(book => (
                  <tr key={book.id} className="border-t hover:bg-amber-50/30 transition-colors" style={{ borderColor: '#F0EBE3' }}>
                    ...
                    <td className="px-4 py-3 hidden md:table-cell">
                      <span className="text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: '#F0EBE3', color: '#8B4513' }}>{book.language}</span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button onClick={() => setEditingBook(book)}
                          ...
                          style={{ borderColor: '#E8E0D5', color: '#8B4513' }}>Edit</button>
          ...
          <div className="mt-4 flex items-center justify-between text-sm">
            ...
            <div className="flex gap-2">
              <button disabled={page === 1} onClick={() => setPage(p => p - 1)}
                ...
                style={{ borderColor: '#E8E0D5' }}>← Prev</button>
              <button disabled={page === totalPages} onClick={() => setPage(p => p + 1)}
                className="px-3 py-1.5 rounded-lg border text-xs disabled:opacity-40 transition-colors hover:bg-gray-50"
                style={{ borderColor: '#E8E0D5' }}>Next →</button>
            </div>
          </div>
        )}
      </div>
